...
import static com.xiaosw.library.utils.HanziToPinyin.PinyinType.FIRST_HANZI_FIRST_LETTER;
import static com.xiaosw.library.utils.HanziToPinyin.PinyinType.WHOLE_PUT_TOGETHER;
...
public class HanziToPinyin {
    /** @see HanziToPinyin#getClass().getSimpleName() */
    private static final String TAG = "xiaosw-HanziToPinyin";
    /** 如果非汉字，或转化失败，默认返回此值 */
    public static final String DEFAULT_CONVERSION_VALUE = "*";
    /** 汉字｜全拼｜所有汉字首字母｜第一个汉字首字母｜第一个汉字全拼｜最后一个汉字全拼｜最后一个汉字首字母 */
    public static final String[][] mHeteronym = new String[][] {
        {"长沙", "CHANG", "SHA"}, 
        {"长沙市", "CANG", "SHA", "SHI"},
        {"重庆", "CHONG", "QING"},
        {"重庆市", "CHONG", "QING", "SHI"},
        ...
        synchronized (HanziToPinyin.class) {
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINA)) {
                    ...
                    LogUtil.d("Self validation. Result: " + doSelfValidation());
                    sInstance = new HanziToPinyin(true);
                    return sInstance;
                }
            }
            LogUtil.w("There is no Chinese collator, HanziToPinyin is disabled");
    ...
    private static boolean doSelfValidation() {
        ...
        for (char c : UNIHANS) {
            ...
            if (cmp >= 0) {
                ...
                LogUtil.e("Internal error in Unihan table. "
...
//        }
    ...
    /**
     * @Title: getPinyin 
     * @Description: 汉字转为拼音（将根据不同模式转换）
     * @author Xiaoshiwang
     * @create data 2013-10-29
     * @param input
     * @param type 转换模式{@link PinyinType}
     * @return
     * @return: String
     * @throws
     */
    public static String getPinyin(String input, PinyinType type) {
        if(null == input || input.trim().length() == 0) {
            return "请输入汉字";
        }
        return hanziToPinyin(input, type);
    }
    
    /**
     * @Title: getPinyin 
     * @Description: 默认模式转换（ 全拼）
     * @author Xiaoshiwang
     * @create data 2013-10-29
     * @param input
     * @return
    ...
    public static String getPinyin(String input) {
        return hanziToPinyin(input, WHOLE_PUT_TOGETHER);
    }

    /**
     *
     * @param input 汉字
     * @param type {@lnk PinyinType}
     * @return
     */
    private static String hanziToPinyin(String input, PinyinType type) {
        ...
        if (tokens != null && tokens.size() > 0) {
            ...
            for (Token token : tokens) {
                String heteronym = isHeteronym(input, index);
                index ++;
                if (Token.PINYIN == token.type) {
                    switch (type) {
                        case WHOLE_PUT_TOGETHER:
                            if(null == heteronym) {
                        ...
                        case WHOLE_PUT_TOGETHER_FIRST_LETTER:
                            if(null == heteronym) {
                                sb.append(token.target.substring(0, 1));
                            } else {
                                sb.append(heteronym.substring(0, 1));
                            }
                            break;
                            
                        case FIRST_HANZI_FIRST_LETTER:
                            if(null == heteronym) {
                                sb.append(token.target.substring(0, 1));
                            } else {
                                sb.append(heteronym.substring(0, 1));
                        ...
                        case FIRST_HANZI_FIRST_WHOLE_PUT_TOGETHER_LETTER:
                            if(null == heteronym) {
                                sb.append(token.target);
                            } else {
                                sb.append(heteronym);
                        ...
                        case LAST_HANZI_FIRST_LETTER:
                            if(token.source.equals(input.substring(input.length() - 1))) {
                                if(null == heteronym) {
                                    sb.append(token.target.substring(0, 1));
                                } else {
                                    sb.append(heteronym.substring(0, 1));
                        ...
                        case LAST_HANZI_FIRST_WHOLE_PUT_TOGETHER_LETTER:
                            if(token.source.equals(input.substring(input.length() - 1))) {
                                if(null == heteronym) {
                                    sb.append(token.target);
                                } else {
                    ...
                    if(FIRST_HANZI_FIRST_LETTER == type) {
//                        return getResult(sb.append(input.substring(0, 1)));
                        String substring = input.substring(0, 1);
                        if(isLatter(substring)) {
                            return getResult(sb.append(input.substring(0, 1)));
                        }
                        return getResult(sb.append(DEFAULT_CONVERSION_VALUE));
                    } else {
                        return getResult(sb.append(input));
                    }
                } else {
    ...
    /**
     * @Method {@link #isLatter}
     * @Description: 盘点一个字符是否为字母    
     * @param str
     * @return
     * @return boolean
     *
     * @date 2015-12-29 下午10:01:01
     * @Author xiaoshiwang
     */
    ...
    private static String isHeteronym(String input, int index) {
//        LogUtil.e(input + "----------> " + index);
        for (int i = 0; i < mHeteronym.length; i++) {
            if(mHeteronym[i][0].equals(input)) {
                return mHeteronym[i][index+1];
            }
        }
        return null;
    }
     ...
     * @throws
     */
    private static String getResult(StringBuilder sb) {
    ...
    public enum PinyinType {
        ...
        WHOLE_PUT_TOGETHER,
        /** 所有汉字首字母  */
        WHOLE_PUT_TOGETHER_FIRST_LETTER,
        /** 第一个汉字首字母 */
        FIRST_HANZI_FIRST_LETTER,
        /** 第一个汉字拼音  */
        FIRST_HANZI_FIRST_WHOLE_PUT_TOGETHER_LETTER,
        /** 最后一个汉字首字母  */
        LAST_HANZI_FIRST_LETTER,
        /** 最后一个汉字全拼  */
        LAST_HANZI_FIRST_WHOLE_PUT_TOGETHER_LETTER;
    }
